...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  _addAPIEventHandlers() {
    ...
    const callTool = async (tool) => {
      try {
        ...
        if (!toolConfig) {
          console.log("toolConfig not found. Its ok if thhis code is running as a relay")
          return
          // throw new Error(`Tool "${tool.name}" has not been added`);
        }
        const result = await toolConfig.handler(jsonArguments);
        if(!toolConfig.skipTrigger){ 
   ...
   */
  addTool(definition, handler, skipTrigger = false) {
    if (!definition?.name) {
    ...
    this.tools[name] = { definition, handler, skipTrigger };
    this.updateSession();
  ...
  updateSession({
    ...
    language !== void 0 && (this.sessionConfig.language = language);
    max_response_output_tokens !== void 0 &&
      (this.sessionConfig.max_response_output_tokens =
        max_response_output_tokens);
    voice_settings !== void 0 &&
      (this.sessionConfig.voice_settings = voice_settings);
    keywords !== void 0 && (this.sessionConfig.keywords = keywords);
